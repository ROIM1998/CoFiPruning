...
def main():
    sys.argv = ['test_memory_usage.py',
            '--output_dir',
            './output/test_memory_usage/',
            '--model_name_or_path',
            'bert-base-uncased',
            '--task_name',
            'sst2',
            ...
            '--do_distill',
            '--do_layer_distill',
            '--layer_distill_version',
            '4',
            '--report_to',
            'none',
            '--distillation_path',
            '/data0/bowen/minus/bert_base_sst2_best_model',
            ]
    ...
    for layer in range(model.config.num_hidden_layers):
        ...
        model.bert.encoder.layer[layer].attention.self.query = lora.Linear(q.in_features, q.out_features, r=8, lora_alpha=16).to(trainer.args.device)
        model.bert.encoder.layer[layer].attention.self.value = lora.Linear(v.in_features, v.out_features, r=8, lora_alpha=16).to(trainer.args.device)
        ...
        model.bert.encoder.layer[layer].intermediate.dense = lora.Linear(i.in_features, i.  out_features, r=8, lora_alpha=16).to(trainer.args.device)
    ...
    print('Pre-input CUDA memory allocated:', torch.cuda.max_memory_allocated() / MB)
    with torch.no_grad():
        ...
        teacher_outputs = trainer.teacher_model(**teacher_inputs)
    ...
    loss.backward()
    print('Distillation CUDA memory allocated:', torch.cuda.max_memory_allocated() / MB)
    torch.cuda.reset_peak_memory_stats()
    
    loss = trainer.compute_loss(model, inputs)
    loss.backward()
    print('SFT CUDA memory allocated:', torch.cuda.max_memory_allocated() / MB)