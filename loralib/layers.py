...
class PruningLinear(Linear):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if out_retained_indices is None:
        ...
        if in_retained_indices is None:
    ...
    def train(self, mode: bool = True):
        ...
        if self.merge_weights and self.merged and mode:
            # Make sure that the weights are not merged
            if self.r > 0:
                transformed_lora = self.lora_B @ self.lora_A
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0 and not self.merged:
            ...
            if self.r > 0 and self.act_fn is not None:
                lora_A_use = self.lora_A.T if self.bottleneck_mask is None else self.lora_A.T * self.bottleneck_mask
                lora_A_use = (self.input_mask * lora_A_use.T).T if self.input_mask is not None else lora_A_use
                lora_A_use = self.in_transformation @ lora_A_use if self.in_transformation is not None else lora_A_use
                
                lora_B_use = self.lora_B.T * self.output_mask if self.output_mask is not None else self.lora_B.T
                lora_B_use = lora_B_use @ self.out_transformation if self.out_transformation is not None else lora_B_use
                ...
                lora_A_use = self.lora_A.T if self.bottleneck_mask is None else self.lora_A.T * self.bottleneck_mask
                transformerd_lora = lora_A_use @ self.lora_B.T
                if self.output_mask is not None:
                    transformerd_lora = transformerd_lora * self.output_mask
                if self.input_mask is not None:
                    # Treating the input_mask as a psuedo-diagonal matrix. Don't use the broadcasting method directly.
                    transformerd_lora = (self.input_mask * transformerd_lora.T).T
                if self.out_transformation is not None:
                    transformerd_lora = transformerd_lora @ self.out_transformation
                if self.in_transformation is not None:
                    transformerd_lora = self.in_transformation @ transformerd_lora
                additive_result = (self.lora_dropout(x) @ transformerd_lora) * self.scaling
                result += additive_result
            return result
        else:
            return F.linear(x, T(self.weight), bias=self.bias)
        
    def set_grafting_mask(self):
        self.input_mask = nn.Parameter(torch.ones(self.lora_A.shape[1], dtype=torch.float32, device=self.weight.device))
        self.output_mask = nn.Parameter(torch.ones(self.lora_B.shape[0], dtype=torch.float32, device=self.weight.device))
        self.bottleneck_mask = nn.Parameter(torch.ones(self.r, dtype=torch.float32, device=self.weight.device))
    ...
    def calculate_pruned_bottleneck_weights(self, pruned_bottleneck_dim: List[int]):
        def T(w):
            return w.T if self.fan_in_fan_out else w
        pruned_lora_A = self.lora_A.index_select(0, torch.tensor(pruned_bottleneck_dim).int().to(self.lora_A.device))
        pruned_lora_B = self.lora_B.index_select(1, torch.tensor(pruned_bottleneck_dim).int().to(self.lora_B.device))
        if self.r > 0:
    ...
    def calculate_pruned_outdim_weights(self, pruned_out_dim: Optional[List[int]] = None, pruned_bottleneck_dim: Optional[List[int]] = None, pruned_in_dim: Optional[List[int]] = None):
        if self.act_fn is not None:
            ...
            return calculate_pruned_equivalent_weights(self, self.lora_A, self.lora_B, self.r, self.scaling, self.in_transformation, self.out_transformation, pruned_out_dim, pruned_bottleneck_dim, pruned_in_dim)
    ...
    def log_history(self, step: int, pruned_out_dim: Optional[List[int]] = None, pruned_bottleneck_dim: Optional[List[int]] = None, pruned_in_dim: Optional[List[int]] = None, target_r: Optional[int] = None):
        self.history.append({
            ...
            'weight': self.weight.clone().detach().cpu(),
            'lora_A': self.lora_A.clone().detach().cpu(),
            'lora_B': self.lora_B.clone().detach().cpu(),
            'scaling': self.scaling,
            'in_transformation': self.in_transformation.clone().detach().cpu() if self.in_transformation is not None else None,
            'out_transformation': self.out_transformation.clone().detach().cpu() if self.out_transformation is not None else None,
    ...
    def restore_dims(self):
        ...
        if self.lora_A is None and self.lora_B is None:
            self.eval()
            self.lora_A, self.lora_B = nn.Parameter(history_use['lora_A'].clone().detach().to(self.weight.device)), nn.Parameter(history_use['lora_B'].clone().detach().to(self.weight.device))
    ...
    def forward(self, x: torch.Tensor, use_teacher: bool = False):
        ...
        if ((self.r > 0 and not use_teacher) or (self.teacher_r > 0 and use_teacher)) and not self.merged:
            ...
            if self.teacher_r > 0 and use_teacher:
                transformed_teacher_lora = self.teacher_lora_A.T @ self.teacher_lora_B.T
                if self.teacher_out_transformation is not None:
                    transformed_teacher_lora = transformed_teacher_lora @ self.teacher_out_transformation
                if self.teacher_in_transformation is not None:
                    transformed_teacher_lora = self.teacher_in_transformation @ transformed_teacher_lora
                additive_result = (self.lora_dropout(x) @ transformed_teacher_lora) * self.teacher_scaling
                result += additive_result
